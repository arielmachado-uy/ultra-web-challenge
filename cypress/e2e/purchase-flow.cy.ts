...
describe("Purchase suite", () => {
  ...
  it("Successful purchase of a single product", function () {
    ...
    inventoryPage.getCartCounter().should("have.text", "1");
    ...
    cy.get("@productName").then((name) => cy.log(`Selected product: ${name}`));
    ...
    cy.get("@productPrice").then((price) =>
      cy.log(`Selected product price: ${price}`)
    );
    ...
    invoicePage
      ...
      .invoke("text")
      .then((subTotal) => {
        invoiceValues.itemTotal = Number(
          invoicePage.getPriceFromText(subTotal)
        );
        cy.get("@productPrice").then((storedPrice) => {
    ...
    invoicePage
      ...
      .then((total) => {
        ...
        // Math validations
        expect(invoiceValues.total.toString()).to.be.equal(
          (invoiceValues.itemTotal + invoiceValues.tax).toFixed(2)
        );
      });
